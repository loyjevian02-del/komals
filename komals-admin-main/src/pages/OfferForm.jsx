import { useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { api } from '../lib/api.js';
import { ImageUploader } from '../components/ImageUploader.jsx';

const empty = { title: '', description: '', image: '', badge: '', showText: false, rank: 0, active: true, displayType: 'SCROLLABLE' };

export default function OfferForm() {
  const { id } = useParams();
  const isNew = id === 'new';
  const navigate = useNavigate();
  const [form, setForm] = useState(empty);

  useEffect(() => {
    if (!isNew) {
      api.get(`/admin/offers/${id}`).then((r) => setForm(r.data));
    }
  }, [id, isNew]);

  function set(key, value) {
    setForm((f) => ({ ...f, [key]: value }));
  }

  async function save(e) {
    e.preventDefault();
    const payload = { ...form, rank: Number(form.rank) || 0 };
    if (isNew) {
      await api.post('/admin/offers', payload);
    } else {
      await api.put(`/admin/offers/${id}`, payload);
    }
    navigate('/offers');
  }

  return (
    <div>
      <h1 style={{ fontSize: 20, marginBottom: 20 }}>{isNew ? 'New Offer' : 'Edit Offer'}</h1>
      <form onSubmit={save} className="card" style={{ maxWidth: 480 }}>
        <div className="field">
          <label>Title</label>
          <input value={form.title} onChange={(e) => set('title', e.target.value)} />
        </div>
        <div className="field">
          <label>Description</label>
          <textarea value={form.description || ''} onChange={(e) => set('description', e.target.value)} />
        </div>
        <div className="field">
          <label>Badge (e.g. "20% OFF")</label>
          <input value={form.badge || ''} onChange={(e) => set('badge', e.target.value)} />
        </div>
        <div className="field">
          <label>Image</label>
          <ImageUploader value={form.image} onChange={(url) => set('image', url)} />
        </div>
        {form.image && (
          <div className="field" style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <input type="checkbox" id="showText" checked={!!form.showText} onChange={(e) => set('showText', e.target.checked)} style={{ width: 'auto' }} />
            <label htmlFor="showText" style={{ margin: 0 }}>Show title/badge/description on storefront (otherwise image only)</label>
          </div>
        )}
        <div className="field">
          <label>Section</label>
          <select value={form.displayType} onChange={(e) => set('displayType', e.target.value)}>
            <option value="FIXED">Fixed banner (single, static)</option>
            <option value="SCROLLABLE">Auto-scrollable (multiple, carousel)</option>
          </select>
        </div>
        <div className="field">
          <label>Rank (display order)</label>
          <input type="number" value={form.rank} onChange={(e) => set('rank', e.target.value)} />
        </div>
        <div className="field" style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <input type="checkbox" id="active" checked={form.active} onChange={(e) => set('active', e.target.checked)} style={{ width: 'auto' }} />
          <label htmlFor="active" style={{ margin: 0 }}>Active (visible on storefront)</label>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="submit" className="btn btn-primary">Save</button>
          <button type="button" className="btn btn-secondary" onClick={() => navigate('/offers')}>Cancel</button>
        </div>
      </form>
    </div>
  );
}
